 ...
 * 
 *         Responsibilities of class: It sorts a Book array by price of its
...
// A SortByPrice is Sortable.
public class SortByPrice implements Sortable
{
	// A SortByRating has many sortedBooksByPrice;
	private Book[] sortedBooksByPrice;

	public SortByPrice(Book[] books)
	{
		// Assign the sortedBooksByPrice by invoking the bubbleSort method and books as its argument.
		sortedBooksByPrice = bubbleSort(books);
	}

	/**
	 * Purpose: Bubble sort algorithm sorts by books Price.
	 * 
	...
	public Book[] bubbleSort(Book[] books)
	{
		...
		while (!inOrder(books))
		{
			for (int i = 0; i < books.length - 1; i++)
			{
				...
				// Check if an element's value in the books with index i is bigger than the
	 ...
	 * by Price.
	 * 
	 * @param books - To check if its elements are in order by Price.
	 * @return true if the books array is sorted by Price, false if it's not.
	 */
	public boolean inOrder(Book[] books)
	{
		for (int i = 0; i < books.length - 1; i++)
		{
			// Check if an element's value in books with index i is bigger than an
			// next element in the books.
			if (books[i].getPrice() > books[i + 1].getPrice())
	...
	/**
	 * Purpose: Get the sorted books by Price.
	 * 
	 * @return books
	 */
